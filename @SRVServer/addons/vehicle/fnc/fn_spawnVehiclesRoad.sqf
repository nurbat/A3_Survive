{
	...
	while { _spawnedVehicles select 1 <= _numToSpawn } do
	{
		try
		{
			private _position = [[SRVVehicle_cfg_spawnRadius select 0, SRVVehicle_cfg_spawnRadius select 1], SRVVehicle_cfg_spawnRadius select 2] call SRVCore_fnc_findRoadPosition;
			private _positionReal = [_position, 25, SRVVehicle_cfg_roadRadiusSpawn, 5, 0 , 1 , 0 , SRVVehicle_blacklistedPositions] call BIS_fnc_findSafePos;
...
} forEach SRVVehicle_cfg_RandomRoad;