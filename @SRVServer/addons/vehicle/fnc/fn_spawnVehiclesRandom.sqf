{
	...
	while { (_spawnedVehicles select 1) <= _numToSpawn } do
	{
		...
		{
			private _vehObject = "";
			private _positionReal = [[(SRVVehicle_cfg_spawnRadius select 0), (SRVVehicle_cfg_spawnRadius select 1)], 25, (SRVVehicle_cfg_spawnRadius select 2), 5, 0 , 1 , 0 , SRVVehicle_blacklistedPositions] call BIS_fnc_findSafePos;
			if(surfaceIsWater _positionReal) throw false;
			_positionReal = _positionReal + [0];
			if(_persist) then { _vehObject = [_name, selectRandom _classNames, _positionReal, random 360] call SRVVehicle_fnc_createPersistentVehicle; }
			else { _vehObject = [selectRandom _classNames, _positionReal] Call SRVVehicle_fnc_createNonPersistentVehicle; };
