...
if ((getPlayerUID _player) Call SRVServer_core_player_exist) then { //Exist player on sql
    if(spawnDebug) then { diag_log format["[SRVS-RESPAWN] Load: %1", name _player]; };
    _data = (getPlayerUID _player) Call SRVServer_core_player_load; //SQL LOAD || Загрузка пользователя
...
//Если игрока в бд нет, создаем нового по уст.шаблону
	...
	[(getPlayerUID _player), name _player] Call SRVServer_core_player_create;//SRV_Core: Create Player
	sleep 2;
